...
describe('FormValidator', () => {
  const parent: DataManagerDataObject = dataProcessorResultJson.filter(
    (item: DataManagerDataObject) => item.id === 'PF0001-PF1116'
  )[0];
  ...
  describe('Validate', () => {
    test('should fail on sign language interpreter radio other validation', async () => {
      ...
      expect(validationErrors).toHaveProperty('other-text-area');
