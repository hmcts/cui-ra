...
describe('FormValidator', () => {
  const parent: DataManagerDataObject = dataProcessorResultJson.filter(
    (item: DataManagerDataObject) => item.id === 'PF0001-RA0001-RA0008-RA0043'
  )[0];
  ...
  describe('Validate', () => {
    test('should fail on sign language interpreter checkbox other validation', async () => {
      ...
      expect(validationErrors).toHaveProperty('other');
      expect(validationErrors.other).toContain('.OT0001.error.');
    });

    test('should pass on sign language interpreter checkbox other validation', async () => {
