...
export class FormValidator {
    ...
    for (let i = 0; i < processedItems.length ; i++) {
        ...
        if(!isValid){
            ...
            if(errors && errors[0] && errors[0].message){
                let message = `${key}.${errors[0].message}`;
  ...
  public static async validateBody(flag:DataManagerDataObject, form: Form): Promise<[boolean,{[key: string]: string}]> {
    const ajv: Ajv = new Ajv({ allErrors: true });
    addFormats(ajv);
    addErrors(ajv);
    ...
    let key = `${flag.value.flagCode}.error`;
